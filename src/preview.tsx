// Standalone preview entry point — loads screens with NO legacy CSS.
// Only loads Tailwind + V6 animations. No auth, no Supabase, no legacy styles.
// URL params: ?screen=home (default) | ?screen=panel | ?screen=prepare-summary
//            | ?screen=daily-summary | ?screen=performance-review

import React, { useCallback, useEffect, useMemo, useRef, useState } from 'react';
import { createRoot } from 'react-dom/client';
import './preview.css';
import { HomeScreen } from './components/screens/HomeScreen';
import { FullscreenPanelScreen } from './components/screens/FullscreenPanelScreen';
import { PrepareSummaryScreen } from './components/screens/PrepareSummaryScreen';
import { DailyWorkSummaryScreen } from './components/screens/DailyWorkSummaryScreen';
import { PerformanceReviewScreen } from './components/screens/PerformanceReviewScreen';
import { PickPanelScreen } from './components/screens/PickPanelScreen';
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          {(isHome || isSettings || isProfile) && (
            <nav className="md:hidden bg-white border-t border-slate-100 px-2 pb-6 pt-2 flex items-center justify-around shrink-0">
              ...
              <button onClick={() => navigate('prepare-summary')} className="flex flex-col items-center gap-0.5 px-3 py-1 text-slate-400">
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
